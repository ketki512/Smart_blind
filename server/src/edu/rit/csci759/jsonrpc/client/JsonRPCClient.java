...
public class JsonRPCClient {
	...
	public static void main(String[] args) {
		...
		try {
			serverURL = new URL("http://10.10.10.117:8080");
...
//		System.out.println(response.getResult());
		String output = response.getResult().toString();
		String[] values = output.split(" ");
		double tempVal = Double.parseDouble(values[1]);
		System.out.println("Temperature value : " + tempVal);
//		
		method = "getAmbience";
		requestID++;
		...
		double ambienceVal = Double.parseDouble(response.getResult().toString());
		System.out.println("Ambience value : " + ambienceVal);
		
		MyTipperClass mtc = new MyTipperClass();
		mtc.setTemp(tempVal);
		mtc.setAmbience(ambienceVal);
		mtc.deFuzzify();
